...
alert("You have selected"+" "+ selectedText + " " + "Employee Id for display the details");
return true;
}
</script>
<style>
...
<form class="row g-3 needs-validation" action="PerformFind" method="post" novalidate id="s"  name="form" onSubmit="return validate()">
...
<c:forEach items="${idList}" var="id"><option>${id}</option></c:forEach></select>
  ...
  <div class="col-sm-5" id="l">
    <button class="btn btn-primary" type="submit" value="Find Employee">Find Employee</button>
    </div>
    </div>
</form>
</body>
